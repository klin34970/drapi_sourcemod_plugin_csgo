...
{
        name = PLUGIN_NAME,
        author = "GoD-Tony, DR. API Improvements",
...
public StopSoundCookieHandler(client, CookieMenuAction:action, any:info, String:buffer[], maxlen)
{
        switch (action)
        {
                ...
                case CookieMenuAction_SelectOption:
                {
                        ...
                        {
                                CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] You have no access!");
                        }
                }
        }
}
        ...
        switch(action)
        {
                ...
                case MenuAction_Select:
                {
                        ...
                        if( GetMenuItem(menu, param2, info, sizeof(info)) )
                        {
                                ...
                                CPrintToChat(param1, "[{GREEN}TIMER{NORMAL}] Weapon sounds %s.", g_bStopSound[param1] ? "disabled" : "enabled");
                                CheckHooks();
                                PrepareMenu(param1);
                        }
                }
...
public Action:Command_StopSound(client, args)
{
        ...
        {
                CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] Your Cookies are not yet cached. Please try again later...");
        }
