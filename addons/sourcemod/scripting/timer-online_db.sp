...
new String:g_sAuth[MAXPLAYERS + 1][64];
...
ConnectSQL()
{
	...
	{
		SQL_TQuery(g_hSQL, CreateSQLTableCallback, "CREATE TABLE IF NOT EXISTS `online` (`auth` varchar(64) NOT NULL, `server` int(11) NOT NULL, UNIQUE KEY `online_single` (`auth`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;");
		RefreshTable();
	}
}
