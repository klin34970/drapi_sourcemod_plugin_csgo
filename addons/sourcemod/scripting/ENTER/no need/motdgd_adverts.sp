...
public OnPluginStart()
{
	...
	g_Preload = CreateConVar("sm_motdgd_preload", "0", "Maximum seconds allowed for preloading the advertisement (if supported), a setting of 0 disables preloading");
	g_Chat = CreateConVar("sm_motdgd_chat", "1", "Enable chat advertisements");
	// Global Server Variables //
...
public Action:Event_End(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarFloat(g_Review) < 15.0 || (GetConVarInt(g_OnOther) != 1 && GetConVarInt(g_OnOther) != 3 && GetConVarInt(g_OnOther) != 5 && GetConVarInt(g_OnOther) != 7))
...
public Action:Event_Start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// Re-view minutes must be 15 or higher, re-view mode (onother) for this event
	if (GetConVarFloat(g_Review) < 15.0 || (GetConVarInt(g_OnOther) != 4 && GetConVarInt(g_OnOther) != 5 && GetConVarInt(g_OnOther) != 6 && GetConVarInt(g_OnOther) != 7))
		return Plugin_Continue;
...
public Action:CheckPlayerDeath(Handle:timer, any:userid)
{
	...
	// Re-view minutes must be 15 or higher, re-view mode (onother) for this event
	if (GetConVarFloat(g_Review) < 15.0 || (GetConVarInt(g_OnOther) != 2 && GetConVarInt(g_OnOther) != 3 && GetConVarInt(g_OnOther) != 6 && GetConVarInt(g_OnOther) != 7))
		return Plugin_Stop;
...
public Action:ChatAdTimer(Handle:timer)
{
	...
	if (GetConVarBool(g_Chat) && GetRealPlayerCount() > 0)
	{
