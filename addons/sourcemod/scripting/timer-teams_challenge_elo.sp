...
{
	LoadTranslations("drapi/drapi_timer-teams_challenges_elo.phrases");
	RegConsoleCmd("sm_pvpstats", Cmd_PvPStats);
...
public ConnectSQLCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (StrEqual(driver, "mysql", false))
	{
		...
		SQL_TQuery(g_hSQL, CreateSQLTableCallback, "CREATE TABLE IF NOT EXISTS `pvp_challenge` (`id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `map` varchar(32) NOT NULL, `style` int(8) NOT NULL, `bonus` int(8) NOT NULL, `auth_winner` varchar(64) NOT NULL,`name_winner` varchar(64) NOT NULL,`auth_loser` varchar(64) NOT NULL,`name_loser` varchar(64) NOT NULL,`diff` int(8) NOT NULL,`count` int(8) NOT NULL, date TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY `single_auth` (`map`,`style`,`bonus`,`auth_winner`,`auth_loser`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;");
		SQL_TQuery(g_hSQL, CreateSQLTableCallback, "CREATE TABLE IF NOT EXISTS `pvp_elo` (auth varchar(64) NOT NULL, name varchar(64) NOT NULL, rating int(8) NOT NULL, rating_max int(8) NOT NULL, rating_min int(8) NOT NULL,win int(8) NOT NULL, loose int(8) NOT NULL,UNIQUE KEY `single_auth` (`auth`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;");
	}
...
public Action:Cmd_PvPStats(client, args)
{
	if (client)
	{
		if(g_iRank[client] != -1)
		{
			CPrintToChatAll("%t", "Rating", client, g_iELO[client], g_iRank[client], g_iRankTotal);
		}
		else 
		{
			CPrintToChat(client, "%t", "No Rating");
		}
	}
...
public OnChallengeWin(winner, loser)
{
	...
	CPrintToChatAll("%t", "Win", winner, g_iELO[winner], g_iELO[winner], loser, g_iELO[loser], g_iELO[winner]);
		
	GetCurrentMap(map, sizeof(map));
	
	GetClientAuthId(winner, AuthId_Steam2, auth_winner, sizeof(auth_winner));
	GetClientAuthId(loser, AuthId_Steam2, auth_loser, sizeof(auth_loser));
	...
	{
		//Loser
		FormatEx(query, sizeof(query), "INSERT INTO pvp_elo (`auth`, `name`, `rating`, `rating_max`, `rating_min`, `loose`) VALUES ('%s', '%s', '%d', '%d', '%d', '%d') ON DUPLICATE KEY UPDATE `name` = '%s', `lose` = `lose`+'1', `rating` = '%d', `rating_min` = '%d';", auth_loser, name_loser, g_iELO[loser], g_iELO[loser], 1000, 0, 1, g_iELO[loser], g_iELOmin[loser]);
		SQL_TQuery(g_hSQL, PvPChallengeCallback, query, _, DBPrio_High);
	}
