...
{
	name = "DR.API je te tiens par la barbichette",
	author = "Dr. Api",
	description = "DR.API je te tiens par la barbichette by Dr. Api",
...
public void OnPluginStart()
{
	...
	AutoExecConfig_CreateConVar("drapi_barbichette_version", PLUGIN_VERSION, "Version", CVARS);
	
	cvar_active_knives_dev			= AutoExecConfig_CreateConVar("drapi_active_barbichette_dev", 			"0", 					"Enable/Disable Dev Mod", 																			DEFAULT_FLAGS, 		true, 0.0, 		true, 1.0);
	
	cvar_knives_ct_default			= AutoExecConfig_CreateConVar("drapi_barbichette_ct_default", 			"512", 					"Default barbichette for CT", 																			DEFAULT_FLAGS);
	cvar_knives_t_default			= AutoExecConfig_CreateConVar("drapi_barbichette_t_default", 			"512", 					"Default barbichette for T", 																				DEFAULT_FLAGS);
	
	cvar_knives_storage_mode		= AutoExecConfig_CreateConVar("drapi_barbichette_storage_mode", 			"2", 					"0=Disable / 1=Cookie / 2=SQL", 																	DEFAULT_FLAGS);
	
	cvar_knives_access_mode			= AutoExecConfig_CreateConVar("drapi_barbichette_access_mode", 			"0", 					"0=All / 1=VIP / 2=ADMIN", 																			DEFAULT_FLAGS);
	
	cvar_knives_give_mode			= AutoExecConfig_CreateConVar("drapi_barbichette_give_mode", 			"1", 					"0=Don't give the barbichette / 1=Give the barbichette", 														DEFAULT_FLAGS);
	
	cvar_knives_check_knife_mode	= AutoExecConfig_CreateConVar("drapi_barbichette_check_knife_mode", 		"1", 					"Check the barbichette before giving away. SUCCESS if barbichette doesn't have a name", 						DEFAULT_FLAGS);
	
	HookEvents();
	
	AutoExecConfig_ExecuteFile();
	
	RegServerCmd("sm_create_barbichette_tables", 	Command_CreateTables);
