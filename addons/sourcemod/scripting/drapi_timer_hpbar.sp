...
public void OnPluginStart()
{
	...
	AutoExecConfig_CreateConVar("drapi_hp_bar_version", PL_VERSION, "Version", CVARS);
	...
	cvar_hpbar_fadeout_t 						= AutoExecConfig_CreateConVar("drapi_hpbar_fadeout_t", 						"0", 					"Distance to display health bar for T Team", 							DEFAULT_FLAGS);
	cvar_hpbar_fadeout_ct 						= AutoExecConfig_CreateConVar("drapi_hpbar_fadeout_ct", 					"0", 					"Distance to display health bar for CT Team, 0=always", 				DEFAULT_FLAGS);
...
public void Event_PlayerHurt(Handle event, char[] name, bool dontBroadcast)
{
	...
	{
		int style = Timer_GetStyle(victim);
		if(style == 19)
		{
