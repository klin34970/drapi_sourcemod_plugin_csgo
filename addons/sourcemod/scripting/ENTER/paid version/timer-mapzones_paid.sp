...
InitZoneSprites()
{
	...
	//if(!IsModelPrecached(spritebuffer))
	{
		...
		//LogMessage("InitZoneSprites: %s", g_sBeamStartZonePath);
	}
...
public Action:DrawZones(Handle:timer)
{
	for (new zone = 0; zone < g_mapZonesCount; zone++)
	{
		...
		{
			DrawBox(point1, point2, 1.0, g_startColor, false, precache_laser_start); 
			//LogMessage("DrawZones: %f, %f - %i", point1, point2, precache_laser_start);
		}
		else if (g_mapZones[zone][Type] == ZtEnd)
			DrawBox(point1, point2, 1.0, g_endColor, false, precache_laser_end);
			...
			DrawBox(point1, point2, 1.0, g_bonusstartColor, false, precache_laser_bonus_start);
			...
			DrawBox(point1, point2, 1.0, g_bonusendColor, false, precache_laser_bonus_end);
	}
...
SpawnZoneEntitys(zone)
{
	...
	/*if(g_bSpawnSpotlights && g_Settings[ZoneSpotlights])
	{
			...
			g_mapZones[zone][Type] == ZtBonus5Level)
		{
			...
			LogMessage("SpawnZoneSpotLights: %i", zone);
		}
	}*/
}
...
stock SpawnZoneSpotLights(zone)
{
	new Float:fFrom[3];
...
stock SpawnSpotLight(Float:pos[3], Float:color[3], Float:ang[3])
{
	...
	LogMessage("CreateEntityByName: %i, %f", entity, pos);
}
